...
# 4. BLISS to Select Appropriate Spatial Scales and Summary Statistics of Covariates  
...
# 5. Calculate Prior Probabilities for Gibbs Variable Selection (GVS step 1)
```{r, include=FALSE, cache=FALSE}
knitr::read_chunk('04-model-probs-for-priors-GVS_step1.R')
```
```{r, prior probs, eval=FALSE}
```

# 6. Run Global Model to Obtain Estimates for Gibbs Variable Selection (GVS step 2)
```{r, include=FALSE, cache=FALSE}
knitr::read_chunk('05-misclass_GVS_step2.R')
```
```{r, global model, eval=FALSE}
```

# 7. Run Model with Gibbs Variable Selection (GVS step 3)
```{r, include=FALSE, cache=FALSE}
knitr::read_chunk('06-misclass_GVS_step3.R')
```
